...
describe('types.ts - Type usability', () => {
	...
	it('TokenRule interface should be constructable', () => {
	...
	it('TokenRule supports optional nextState and popState', () => {
		const rule: TokenRule = {
			type: 'comment.block',
			pattern: /\/\*/,
		...
		expect(rule.nextState).toBe('blockComment');
		expect(rule.popState).toBe(false);
	});

	it('LanguageGrammar interface should be constructable', () => {
		const grammar: LanguageGrammar = {
			...
			blockComment: ['/*', '*/'],
			rules: {
				root: [{ type: 'keyword', pattern: /\bfoo\b/ }]
			}
		};
		expect(grammar.language).toBe('test');
		expect(grammar.extensions).toContain('.test');
		expect(grammar.blockComment?.[0]).toBe('/*');
	});
});
...
// ============================================================
// base.ts - SimpleTokenizer
// ============================================================

describe('SimpleTokenizer', () => {
	it('should use the provided language name', () => {
		const tokenizer = new SimpleTokenizer('test', []);
		expect(tokenizer.language).toBe('test');
	});

	it('should return empty initial state', () => {
		const tokenizer = new SimpleTokenizer('test', []);
		expect(tokenizer.getInitialState()).toEqual({});
	});

	it('should match tokens using regex patterns', () => {
		const tokenizer = new SimpleTokenizer('test', [
			{ type: 'keyword', regex: /^(const|let|var)\b/ },
			{ type: 'number.integer', regex: /^\d+/ },
			{ type: 'string', regex: /^"[^"]*"/ }
		]);

		const result = tokenizer.tokenizeLine('const 42 "hi"', 1);
		const types = result.tokens.map((t) => t.type);

		expect(types).toContain('keyword');
		expect(types).toContain('number.integer');
	...
	it('should produce text tokens for unmatched characters', () => {
		...
		const numberToken = result.tokens.find((t) => t.type === 'number.integer');
		expect(numberToken).toBeDefined();
		expect(numberToken!.text).toBe('123');

		const textTokens = result.tokens.filter((t) => t.type === 'text');
		expect(textTokens.length).toBeGreaterThanOrEqual(1);
	});

	it('should merge consecutive unmatched characters into a single text token', () => {
		const tokenizer = new SimpleTokenizer('test', []);
		const result = tokenizer.tokenizeLine('abc', 1);
		// All characters should merge into one text token
		expect(result.tokens).toHaveLength(1);
		expect(result.tokens[0].type).toBe('text');
		expect(result.tokens[0].text).toBe('abc');
	});

	it('should produce an empty text token for empty lines', () => {
		const tokenizer = new SimpleTokenizer('test', []);
		const result = tokenizer.tokenizeLine('', 1);
		expect(result.tokens).toHaveLength(1);
		expect(result.tokens[0].type).toBe('text');
	...
	it('should handle block comment start and continuation across lines', () => {
		const tokenizer = new SimpleTokenizer('test', []);

		// Line with block comment start but no end
		const line1 = tokenizer.tokenizeLine('/* open comment', 1);
		expect(line1.state?.inBlockComment).toBe(true);
		expect(line1.tokens[0].type).toBe('comment.block');

		// Continuation line
		const line2 = tokenizer.tokenizeLine('still in comment */', 2, line1.state);
		expect(line2.tokens[0].type).toBe('comment.block');
		expect(line2.state?.inBlockComment).toBe(false);
	});

	it('should handle single-line block comments', () => {
		const tokenizer = new SimpleTokenizer('test', []);
		const result = tokenizer.tokenizeLine('/* comment */ rest', 1);
		const commentToken = result.tokens.find((t) => t.type === 'comment.block');
	...
	it('should set correct start/end positions for tokens', () => {
		...
		expect(kw!.start).toBe(2);
		expect(kw!.end).toBe(7);
		expect(kw!.text).toBe('hello');
	});
});

// ============================================================
// base.ts - GrammarTokenizer
// ============================================================

describe('GrammarTokenizer', () => {
	function makeGrammar(overrides: Partial<LanguageGrammar> = {}): LanguageGrammar {
		return {
			language: 'testlang',
			extensions: ['.tl'],
			lineComment: '//',
			...
			rules: {
				root: [
					{ type: 'keyword', pattern: /^(if|else|while|return)\b/ },
					{ type: 'number.integer', pattern: /^\d+/ },
					{ type: 'string', pattern: /^"[^"]*"/ },
					{ type: 'comment.line', pattern: /^\/\/.*/ },
					{ type: 'comment.block', pattern: /^\/\*/, nextState: 'blockComment' }
				],
				blockComment: [
					{ type: 'comment.block', pattern: /^.*?\*\//, popState: true },
					{ type: 'comment.block', pattern: /^.+/ }
				]
			},
			...overrides
		};
	...
	it('should use the grammar language name', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		expect(tokenizer.language).toBe('testlang');
	});
	...
	it('should tokenize keywords from grammar rules', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine('if else while return', 1);
		const keywords = result.tokens.filter((t) => t.type === 'keyword');
		expect(keywords).toHaveLength(4);
		expect(keywords[0].text).toBe('if');
		expect(keywords[1].text).toBe('else');
		expect(keywords[2].text).toBe('while');
		expect(keywords[3].text).toBe('return');
	});

	it('should tokenize numbers from grammar rules', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine('42', 1);
		expect(result.tokens[0].type).toBe('number.integer');
		expect(result.tokens[0].text).toBe('42');
	});

	it('should tokenize strings from grammar rules', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine('"hello"', 1);
	...
	it('should produce text tokens for unmatched characters', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine('xyz', 1);
		expect(result.tokens).toHaveLength(1);
		expect(result.tokens[0].type).toBe('text');
		expect(result.tokens[0].text).toBe('xyz');
	});

	it('should handle empty lines with a single empty text token', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine('', 1);
		expect(result.tokens).toHaveLength(1);
		expect(result.tokens[0].type).toBe('text');
	...
	it('should transition into block comment state via nextState', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());

		// Open block comment - nextState should set inBlockComment to true
		const line1 = tokenizer.tokenizeLine('/* comment start', 1);
		expect(line1.state?.inBlockComment).toBe(true);
	});

	it('should start in blockComment rule set when state has inBlockComment', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());

		// When prevState has inBlockComment, the tokenizer starts in the blockComment rule set
		const result = tokenizer.tokenizeLine('still comment */', 2, { inBlockComment: true });
		// The blockComment rules should match and produce comment.block tokens
		expect(result.tokens[0].type).toBe('comment.block');
	});

	it('should switch currentRuleSet back to root on popState', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());

		// After popState, subsequent text on the same line should use root rules
		const result = tokenizer.tokenizeLine('end */ if', 1, { inBlockComment: true });
		// The comment closing should be matched, then 'if' should be tokenized by root rules
		const kwToken = result.tokens.find((t) => t.type === 'keyword');
		expect(kwToken).toBeDefined();
		expect(kwToken!.text).toBe('if');
	});

	it('should merge consecutive unmatched characters', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine('abcdef', 1);
		expect(result.tokens).toHaveLength(1);
		expect(result.tokens[0].text).toBe('abcdef');
	});

	it('should set correct positions on tokens', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine(' if', 1);
		// space is unmatched text, 'if' is keyword
		const kw = result.tokens.find((t) => t.type === 'keyword');
		expect(kw).toBeDefined();
		expect(kw!.start).toBe(1);
		expect(kw!.end).toBe(3);
	});

	it('should return a copy of state (no shared references)', () => {
		const tokenizer = new GrammarTokenizer(makeGrammar());
		const result = tokenizer.tokenizeLine('42', 1);
		const state = result.state!;
		state.inBlockComment = true;
		// Original result state should not be affected since we got a copy
		// This tests that the state is spread, not passed by reference
		expect(result.state?.inBlockComment).toBe(true); // same object since we mutated it
		// The important thing is that next tokenizeLine call gets a fresh copy
		const result2 = tokenizer.tokenizeLine('42', 2, { inBlockComment: false });
		expect(result2.state?.inBlockComment).toBe(false);
	});
});
